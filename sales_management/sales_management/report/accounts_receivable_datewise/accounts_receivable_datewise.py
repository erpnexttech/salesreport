...
class ReceivablePayableReport(object):
    ...
    def get_gl_entries(self):
        ...
        print(values)
        if self.filters.show_future_payments:
            values[1] = self.filters.from_date
            values.insert(2, self.filters.to_date)

            date_condition = """AND (posting_date between %s and %s)
				OR (against_voucher IS NULL AND DATE(creation) between %s and %s))"""
        else:
            values[1] = self.filters.from_date
            values.insert(2, self.filters.to_date)
    ...
    def add_column(self, label, fieldname=None, fieldtype="Currency", options=None, width=120,hidden=0):
        ...
        self.columns.append(
            dict(label=label, fieldname=fieldname,
                 fieldtype=fieldtype, options=options, width=width,hidden=hidden)
        )
    ...
    def setup_ageing_columns(self):
        ...
        for i, label in enumerate(
            ...
            self.add_column(label=label, fieldname="range" + str(i + 1),hidden=1)
